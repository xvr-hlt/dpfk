...
class BlazeFace(nn.Module):
    ...
    def load_anchors(self, path):
        self.anchors = torch.tensor(np.load(path),
                                    dtype=torch.float32,
                                    device=self._device())
    ...
    def predict_on_batch(self, x):
        ...
        for i in range(len(detections)):
            ...
            faces = torch.stack(faces) if len(faces) > 0 else torch.zeros(
                (0, 17))
    ...
    def _decode_boxes(self, raw_boxes, anchors):
        ...
        x_center = raw_boxes[..., 0] / self.x_scale * anchors[:, 2] + anchors[:,
                                                                              0]
        y_center = raw_boxes[..., 1] / self.y_scale * anchors[:, 3] + anchors[:,
                                                                              1]
        ...
        for k in range(6):
            offset = 4 + k * 2
            keypoint_x = raw_boxes[
                ..., offset] / self.x_scale * anchors[:, 2] + anchors[:, 0]
            keypoint_y = raw_boxes[
                ..., offset + 1] / self.y_scale * anchors[:, 3] + anchors[:, 1]
